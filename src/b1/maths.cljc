...
(defn add
  ([& args] (reduce (fn [A B] (combine-with A B + add)) args)))
(defn sub
  ([& args] (if (= (count args) 1)
              (combine-with 0 (first args) - sub)
              (reduce (fn [A B] (combine-with A B - sub)) args))))
(defn mul
  ([& args] (reduce (fn [A B] (combine-with A B * mul)) args)))
(defn div
  ([& args] (if (= (count args) 1)
              (combine-with 1 (first args) / div)
