      ...
      (map (fn [d scale]
             [:g.tick.major-tick {:transform (translate {x 0 y (scale d)})}
              [:text {:text-anchor text-anchor
                      :transform   (translate {x (* parity 5) y 0})
...
#_(defmethod as-svg :histogram [{:keys [histograms bins x-axis y-axis]
  ...
  (let [margin-vertical (if x-axis 40 0)
        ...
        scale-y (scale/linear :domain [0 max-y]
                              :range [(- height margin-vertical)
                                      margin-vertical])]
    [:svg {:width width :height height}
     #_[:g {:transform (translate [margin-horizontal 0])}
        (axis scale-y (:ticks (search [0 max-y])) :orientation :left)]
     [:g {:transform (translate [0 (- height margin-vertical)])}
      (axis scale-x
     ...
     [:g.chart
      (for [bar bars]
        (for [y (range (:y bar))]
          [:g.bar {:class     (str "series-" (:series bar))
                   :transform (translate [(+ (-> bar :x scale-x) ;24
                                             (* bar-width (:series bar) 0.5))
                                          (- height (* tally-height (inc y)) margin-vertical
                                             )])}
           [:rect {:height (* tally-height 0.8)
                   :width  (/ bar-width 2)}]]))]]))

#_(defmethod as-svg :function-area [{:keys [functions x-axis y-axis]} & {:keys [width height]}]
    (let [margin-vertical (if x-axis 40 0)
          margin-horizontal (if y-axis 40 0)
